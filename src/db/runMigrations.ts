...
import { splitSqlStatements } from "./migrationUtils.js";
export { splitSqlStatements } from "./migrationUtils.js";
...
const migrationFilePattern = /^\d{14}_.+/;

export const migrationFilesWithIds = (files: ReadonlyArray<string>) =>
  files
...
export const loadDdfDatabaseMigrations = Effect.fn(
  ...
  return yield* Effect.forEach(migrationFilesWithIds(files), ({ file, id }) =>
    Effect.gen(function* () {
      const fileSystem = yield* FileSystem.FileSystem;
...
export const runDdfDatabaseMigrations = Effect.fn("DdfDatabaseMigrations.run")(
...
const cli = Effect.gen(function* () {
  ...
  return yield* runDdfDatabaseMigrations().pipe(Effect.provide(mergedLayer));
});
