...
const initialSchemaSql = `CREATE TABLE "ddf_media" (
	"media_key" text,
	"resource" text,
	"resource_key" text,
...
const migrationEffect = (migrationSql: string) =>
  Effect.gen(function* () {
...
export const embeddedDdfDatabaseMigrations: Migrator.Loader = Effect.succeed([
  [1, "initial_schema", Effect.succeed(migrationEffect(initialSchemaSql))] as const,
]);
