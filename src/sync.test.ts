    ...
    const result = await runWithHttp(
      syncProperties({
        ...
        sink: {
          ...
          upsertMedia: (media) =>
            Effect.sync(() => calls.push(`media:${media.ResourceRecordKey}`)),
...
describe("syncMembers and syncOffices", () => {
  ...
  it("keeps nextLink replication page failures structured and does not advance watermarks", async () => {
    const http = emptyHttp({
      requestJson: <T = unknown>(path: string) => {
        if (path.startsWith("/odata/v1/Property/PropertyReplication")) {
          return response<T>({
    ...
    const property = await runWithHttp(syncProperties(), http);
    const member = await runWithHttp(syncMembers(), http);
    const office = await runWithHttp(syncOffices(), http);
    ...
    assert.equal(member.errors[0]?.key, "page:https://ddf.test/member-page-2");
    assert.equal(office.errors[0]?.key, "page:https://ddf.test/office-page-2");
  });
});
